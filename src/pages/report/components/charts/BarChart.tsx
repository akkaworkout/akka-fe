...
declare module 'chart.js' {
  interface PluginOptionsByType<TType extends ChartType> {
    exerciseBubblePlugin?: TType extends 'bar' ? BubbleOptions : never
  }
}
...
const bubblePlugin: Plugin<'bar'> = {
  ...
  afterDatasetsDraw(chart) {
    const opt = chart.options.plugins?.exerciseBubblePlugin as BubbleOptions | undefined
    if (!opt) return
...
export default function ExerciseBarChart({
  ...
  const data = {
    ...
    datasets: [
      {
        data: values,
        backgroundColor: labels.map((_, i) => (i === selectedIndex ? activeColor : normalColor)),
        barThickness: 18,
