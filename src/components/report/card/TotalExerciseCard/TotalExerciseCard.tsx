...
export default function TotalExerciseCard() {
  ...
  const maxCount = exerciseList.length ? Math.max(...exerciseList.map((i) => i.count)) : 0
  ...
  return (
    <section className="total-exercise-card">
      <header className="card-header">
        <h2 className="card-title">총 운동</h2>
      </header>
      ...
      <ul className="exercise-list">
        {exerciseList.slice(0, VISIBLE_COUNT).map((item, idx) => {
          ...
          return (
            <li key={`${item.label}-${idx}`} className={`exercise-row ${isMax ? 'highlight' : ''}`}>
